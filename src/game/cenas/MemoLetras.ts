...
export class MemoLetras extends Phaser.Scene {
    ...
    create():void {
        this.cartas=[];this.primeira=undefined;this.bloqueado=false;this.encontrados=0;this.paresEncontrados=[];this.sequencia=0;this.timerPrimeira=undefined;
        ...
        const voltar=this.botao(76,26,124,36,'‹  EDUCAPP',0x74649d,12);voltar.on('pointerdown',()=>this.scene.start('EducApp'));
    ...
    private criarCarta(x:number,y:number,w:number,h:number,grupo:string,item:ItemAssociacao):Carta {
        ...
        carta.frente=this.add.container(0,0).setVisible(false);const tamanhoFonte=item.texto.length>6?18:23;carta.frente.add([this.add.graphics().fillStyle(0x51446a,.14).fillRoundedRect(-w/2,-h/2+6,w,h,20).fillStyle(0xffffff,1).fillRoundedRect(-w/2,-h/2,w,h,20).lineStyle(3,0xa891c7,.65).strokeRoundedRect(-w/2,-h/2,w,h,20),this.add.text(0,0,item.texto,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:`${tamanhoFonte}px`,fontStyle:'bold',color:'#51436f'}).setOrigin(.5)]);
    ...
    private mostrarFeedback(texto:string,cor:number):void {this.feedback.removeAll(true);this.feedback.add([this.add.graphics().fillStyle(0xffffff,.97).fillRoundedRect(-165,-23,330,46,17).lineStyle(2,cor,.5).strokeRoundedRect(-165,-23,330,46,17),this.add.text(0,0,texto,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'13px',fontStyle:'bold',color:`#${cor.toString(16).padStart(6,'0')}`}).setOrigin(.5)]);this.feedback.setAlpha(1);this.time.delayedCall(850,()=>this.feedback.setAlpha(0));}
    private concluir():void {audioJogo.efeito('vitoria');servicoVoz.falar(this.paresEncontrados.join('. '));this.feedback.removeAll(true);this.feedback.add([this.add.graphics().fillStyle(0xffffff,.99).fillRoundedRect(-285,-35,570,70,22).lineStyle(3,0x5c9b72,.65).strokeRoundedRect(-285,-35,570,70,22),this.add.text(-80,0,'TODOS OS PARES!',{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'20px',fontStyle:'bold',color:'#49775a'}).setOrigin(.5)]);const novamente=this.botao(188,0,160,42,'JOGAR DE NOVO',0x5c9270,12);this.feedback.add(novamente);novamente.on('pointerdown',()=>this.scene.restart({dificuldade:this.dificuldade}));this.feedback.setAlpha(0).setScale(.9);this.tweens.add({targets:this.feedback,alpha:1,scale:1,duration:200,ease:'Back.Out'});}
