...
export class Jogo extends Phaser.Scene {
    ...
    private indicadoresProgresso:Phaser.GameObjects.Arc[]=[]; private textoRestantes!:Phaser.GameObjects.Text;
    ...
    private alternarPausa():void {
        ...
        if(this.pausado){
            ...
            const fundo=this.add.rectangle(480,320,960,640,0x102a38,.62).setInteractive();
            const painel=this.add.rectangle(480,320,390,210,0xfffbea,.98).setStrokeStyle(7,this.tema.primaria);
            ...
            const continuar=this.add.text(480,355,'▶  CONTINUAR',{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'21px',color:'#ffffff',backgroundColor:'#4baa68',padding:{x:24,y:12}}).setOrigin(.5).setInteractive({useHandCursor:true});
            continuar.on('pointerdown',()=>this.alternarPausa());this.overlayPausa=this.add.container(0,0,[fundo,painel,titulo,continuar]).setDepth(100);
        }else{
    ...
    private criarAmbiente():void {
        ...
        livres.forEach((grade,i)=>{
            const p=this.mundo(grade),g=this.add.graphics().setDepth(1).setAlpha(.85);
            const x=p.x+Phaser.Math.Between(-15,15),y=p.y+Phaser.Math.Between(-15,15);
            if(this.tema.ambiente==='folha'){
                g.fillStyle(0x4aa85e).fillEllipse(x,y,10,5);g.fillStyle(i%2?0xff78a8:0xffd84f).fillCircle(x+3,y-3,3);
            }else if(this.tema.ambiente==='bolha'){
                g.fillStyle(i%2?0xffd7a1:0xf4f0d0).fillEllipse(x,y,12,7);g.lineStyle(2,0xffffff,.8).strokeEllipse(x,y,12,7);
            }else if(this.tema.ambiente==='poeira'){
                g.fillStyle(0xf0bd43).fillRect(x-7,y-5,14,10);g.lineStyle(1,0xb97925).strokeRect(x-7,y-5,14,10);
            }else if(this.tema.ambiente==='luz'){
                g.fillStyle(0x657080).fillCircle(x,y,7);g.fillStyle(0xb7c1ca).fillCircle(x-2,y-2,2);
            }else{
                g.lineStyle(2,0xffffff,.9);g.lineBetween(x-6,y,x+6,y);g.lineBetween(x,y-6,x,y+6);g.lineBetween(x-4,y-4,x+4,y+4);g.lineBetween(x+4,y-4,x-4,y+4);
            }
        });
        for(let i=0;i<14;i++){
        ...
        this.add.rectangle(480,388,720,470,0xffffff,this.tema.ambiente==='neve'?.08:.025).setDepth(3).setBlendMode(Phaser.BlendModes.ADD);
    }
    ...
    private criarLetras():void {
        ...
        letras.forEach((letra,i)=>{
            ...
            escolhidas.push(p);
            const pos=this.mundo(p);const halo=this.add.circle(pos.x,pos.y,27,this.tema.secundaria,.24).setStrokeStyle(2,0xffffff,.65).setDepth(3);const brilho=this.add.star(pos.x+17,pos.y-17,4,2,5,0xffffff,.9).setDepth(5);const texto=this.add.image(pos.x,pos.y,`letra-${letra}`).setDisplaySize(46,46).setDepth(4) as LetraMapa;texto.letra=letra;texto.setData('halo',halo);texto.setData('brilho',brilho);this.physics.add.existing(texto,true);const corpo=texto.body as Phaser.Physics.Arcade.StaticBody;corpo.setCircle(20);corpo.updateFromGameObject();this.physics.add.overlap(this.rato,texto,()=>this.tentarLetra(texto));this.tweens.add({targets:[texto,halo],y:pos.y-6,yoyo:true,repeat:-1,duration:950+i*55,ease:'Sine.InOut'});this.tweens.add({targets:texto,angle:{from:-3,to:3},yoyo:true,repeat:-1,duration:1250+i*35,ease:'Sine.InOut'});this.tweens.add({targets:halo,scale:{from:.82,to:1.12},alpha:{from:.12,to:.35},yoyo:true,repeat:-1,duration:800+i*60});this.tweens.add({targets:brilho,scale:{from:.3,to:1.4},angle:180,alpha:{from:0,to:1},yoyo:true,repeat:-1,duration:700,delay:i*170});
        });
    }
    ...
    private tentarLetra(obj:LetraMapa):void {if(obj.coletada||!obj.letra||this.terminou)return;if(!this.sistemaPalavra.tentar(obj.letra))return;audioJogo.efeito('letra');obj.coletada=true;(obj.body as Phaser.Physics.Arcade.Body).enable=false;this.atualizarPontos(this.fase.pontosLetra);const indice=this.sistemaPalavra.indiceUltimaLetra;const texto=this.caixasLetras[indice].list[2] as Phaser.GameObjects.Text;texto.setText(obj.letra);this.indicadoresProgresso[indice].setFillStyle(this.tema.primaria);const restantes=this.fase.palavra.length-this.sistemaPalavra.letrasColetadas;this.textoRestantes.setText(`${restantes} restantes`);this.particulas(obj.x,obj.y,this.tema.primaria,18);this.cameras.main.shake(80,.002);this.tweens.add({targets:this.caixasLetras[indice],scale:1.35,yoyo:true,duration:180});const halo=obj.getData('halo') as Phaser.GameObjects.Arc,brilho=obj.getData('brilho') as Phaser.GameObjects.Star;this.tweens.killTweensOf([obj,halo,brilho]);this.tweens.add({targets:[obj,halo,brilho],scale:1.7,alpha:0,angle:120,duration:300,ease:'Back.In',onComplete:()=>{obj.destroy();halo.destroy();brilho.destroy();}});if(this.sistemaPalavra.completa)this.finalizar();else this.mensagem.setText(`Muito bem! Faltam ${restantes} letras.`);}
