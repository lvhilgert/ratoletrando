...
export class VooDragaoReino extends Phaser.Scene {
    ...
    private barra!:Phaser.GameObjects.NineSlice;
    ...
    create():void {
        ...
        painelRaster(this,480,320,960,640,fase.cor,this.fase===2?.3:.16);
        this.criarCeu();
    ...
    update(_tempo:number,delta:number):void {
        ...
        const progresso=Phaser.Math.Clamp(decorrido/fase.duracao,0,1);this.barra.setSize(Math.max(1,360*progresso),12);
    ...
    private criarCeu():void {
        ...
        for(let i=0;i<8;i++){
            // raster-exception: camada atmosférica efêmera, sem interação ou significado próprio.
        ...
        // raster-exception: chuva atmosférica efêmera.
        ...
        // raster-exception: brasas atmosféricas efêmeras.
    ...
    private criarObstaculo():void {
        ...
        // raster-exception: corpo de colisão totalmente invisível; a imagem correspondente é raster.
    ...
    private terminar(vitoria:boolean):void {
        ...
        const ultima=this.fase===FASES_VOO.length-1,titulo=vitoria?(ultima?'TRÊS CÉUS CONQUISTADOS!':'FASE CONCLUÍDA!'):'TENTE OUTRA VEZ',painel=this.add.container(480,320).setDepth(50),fundo=painelRaster(this,0,0,960,640,0x102e29,.86),borda=painelRaster(this,0,0,580,320,vitoria?0xffd45f:0xd06c5f),miolo=painelRaster(this,0,0,570,310,0xfffdf3);
        painel.add([fundo,borda,miolo,this.add.text(0,-92,titulo,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'29px',fontStyle:'bold',color:vitoria?'#3f816c':'#a74e45'}).setOrigin(.5),this.add.text(0,-35,`${FASES_VOO[this.fase].nome}\n${this.pontos} desvios  •  ${this.estrelasColetadas} estrelas`,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'18px',color:'#526a61',align:'center',lineSpacing:8}).setOrigin(.5)]);
    ...
    private mostrarAbertura():void {const fase=FASES_VOO[this.fase],painel=this.add.container(480,320).setDepth(30),dica=this.add.text(0,0,`FASE ${this.fase+1} DE 3\n${fase.nome}\n${fase.mecanica}\n↑ SUBA   •   ↓ DESÇA`,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'22px',fontStyle:'bold',color:'#ffffff',align:'center',lineSpacing:11}).setOrigin(.5);painel.add([painelRaster(this,0,0,520,190,0x133a4c,.88),dica]);this.tweens.add({targets:painel,alpha:0,y:285,delay:1800,duration:350,onComplete:()=>painel.destroy(true)});}
    private criarHUD():void {this.textoVidas=this.add.text(38,35,'❤❤❤',{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'27px',color:'#ff6767',stroke:'#5b302d',strokeThickness:4}).setDepth(20);this.textoEstrelas=this.add.text(40,76,`★ ${this.estrelasColetadas}`,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'19px',fontStyle:'bold',color:'#ffd45f',stroke:'#5b4930',strokeThickness:4}).setDepth(20);this.textoPontos=this.add.text(922,35,`DESVIOS: ${this.pontos}`,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'16px',fontStyle:'bold',color:'#ffffff',stroke:'#244d42',strokeThickness:4}).setOrigin(1,.5).setDepth(20);barraRaster(this,480,34,360,12,0x173d34,.7).setDepth(20);this.barra=barraRaster(this,300,34,1,12,0xffd45f).setOrigin(0,.5).setDepth(20);this.add.text(480,51,`FASE ${this.fase+1}/3  •  ${FASES_VOO[this.fase].nome}`,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'12px',fontStyle:'bold',color:'#fff4b8',stroke:'#244d42',strokeThickness:3}).setOrigin(.5).setDepth(20);const fases=this.add.container(870,76,[botaoRaster(this,0,0,104,34,0x173d34,.78),this.add.text(0,0,'‹ FASES',{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'12px',fontStyle:'bold',color:'#ffffff'}).setOrigin(.5)]).setSize(104,34).setDepth(20).setInteractive({useHandCursor:true});fases.on('pointerdown',()=>this.scene.start('ConfiguracaoReino',{tela:'fases'}));}
    private criarTouch():void {if(!window.matchMedia('(pointer: coarse)').matches)return;const criar=(y:number,rotulo:string,down:()=>void,up:()=>void)=>{const botao=this.add.container(875,y).setDepth(25).setSize(72,72).setInteractive();botao.add([circuloRaster(this,0,0,72,0xffffff,.7),circuloRaster(this,0,0,66,0x315f7a,.78),this.add.text(0,0,rotulo,{fontFamily:'Arial',fontSize:'30px',color:'#ffffff'}).setOrigin(.5)]);botao.on('pointerdown',down).on('pointerup',up).on('pointerout',up);};criar(480,'↑',()=>this.cimaTouch=true,()=>this.cimaTouch=false);criar(560,'↓',()=>this.baixoTouch=true,()=>this.baixoTouch=false);}
    private botao(pai:Phaser.GameObjects.Container,x:number,y:number,texto:string,cor:number,acao:()=>void):void {const botao=this.add.container(x,y).setSize(205,50).setInteractive({useHandCursor:true});botao.add([botaoRaster(this,0,0,208,54,0xffffff,.45),botaoRaster(this,0,0,204,50,cor),this.add.text(0,0,texto,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'13px',fontStyle:'bold',color:'#ffffff'}).setOrigin(.5)]);botao.on('pointerdown',acao);pai.add(botao);}
}
