...
export class CavaleiroReino extends Phaser.Physics.Arcade.Sprite {
    ...
    private criarPoeira(pouso:boolean):void {const agora=this.scene.time.now;if(!pouso&&agora<this.proximaPoeira)return;this.proximaPoeira=agora+150;for(let i=0;i<(pouso?7:2);i++){const p=this.scene.add.ellipse(this.x+Phaser.Math.Between(-18,18),this.y+76,Phaser.Math.Between(5,11),Phaser.Math.Between(3,7),0xc9b58b,.55).setDepth(7);this.scene.tweens.add({targets:p,x:p.x+Phaser.Math.Between(-25,25),y:p.y-Phaser.Math.Between(5,18),alpha:0,scale:1.6,duration:Phaser.Math.Between(240,420),onComplete:()=>p.destroy()});}}
}
