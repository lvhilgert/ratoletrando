...
export class MorcegoReino extends Phaser.Physics.Arcade.Sprite implements InimigoReino {
    ...
    constructor(cena:Phaser.Scene,x:number,y:number,private readonly alvo:Phaser.Physics.Arcade.Sprite){
        ...
        this.origemX=x;this.origemY=y;this.inicio=cena.time.now+Phaser.Math.Between(0,900);this.play('reino-morcego-voar');
    }
    ...
    atualizar():void {
        ...
        else if(this.estado==='mergulho'&&(this.y>this.alvo.y+42||agora>=this.mergulhoAte)){this.estado='retorno';this.play('reino-morcego-voar');}
        ...
        this.setRotation(Phaser.Math.Clamp((this.body as Phaser.Physics.Arcade.Body).velocity.x/700,-.28,.28)).setFlipX((this.body as Phaser.Physics.Arcade.Body).velocity.x>0);
    }
