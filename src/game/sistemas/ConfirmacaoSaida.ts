...
export function confirmarSaidaParaEducApp(cena:Phaser.Scene,opcoes:OpcoesSaida={}):void {
    ...
    const limpar=()=>{bloqueioClique.destroy();zonaContinuar.destroy();zonaSair.destroy();};const fechar=()=>{abertas.delete(cena);limpar();c.destroy(true);if(cena.input.keyboard)cena.input.keyboard.enabled=tecladoAtivo;opcoes.aoCancelar?.();};zonaContinuar.on('pointerover',()=>continuar.setScale(1.035));zonaContinuar.on('pointerout',()=>continuar.setScale(1));zonaSair.on('pointerover',()=>sair.setScale(1.035));zonaSair.on('pointerout',()=>sair.setScale(1));zonaContinuar.on('pointerdown',fechar);zonaSair.on('pointerdown',()=>{abertas.delete(cena);limpar();cena.scene.start('EducApp');});
