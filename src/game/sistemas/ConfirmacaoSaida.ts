...
import { botaoRaster, painelRaster } from './ArteRaster';
...
export function confirmarSaidaParaEducApp(cena:Phaser.Scene,opcoes:OpcoesSaida={}):void {
    ...
    const c=cena.add.container(480,320).setScrollFactor(0).setDepth(2000),bloqueioVisual=painelRaster(cena,0,0,960,640,0x142e2a,.68),sombra=painelRaster(cena,0,9,540,270,0x173b34,.2),borda=painelRaster(cena,0,-1,548,278,0xd6ae61,.9),painel=painelRaster(cena,0,-1,540,270,0xfffdf4),titulo=cena.add.text(0,-78,'SAIR DO JOGO?',{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'27px',fontStyle:'bold',color:'#365f52'}).setOrigin(.5),mensagem=cena.add.text(0,-30,'Tem certeza de que deseja voltar ao EducApp?',{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'16px',color:'#61736c',align:'center',wordWrap:{width:440}}).setOrigin(.5);
    const botao=(x:number,rotulo:string,cor:number)=>{const b=cena.add.container(x,65);b.add([botaoRaster(cena,0,5,210,52,0x24483f,.16),botaoRaster(cena,0,0,210,52,cor),cena.add.text(0,0,rotulo,{fontFamily:'Arial Rounded MT Bold, Arial',fontSize:'13px',fontStyle:'bold',color:'#ffffff'}).setOrigin(.5)]);return b;};
    const continuar=botao(-115,'CONTINUAR JOGANDO',0x4c9773),sair=botao(115,'SAIR PARA O EDUCAPP',0xb66c55);c.add([bloqueioVisual,sombra,borda,painel,titulo,mensagem,continuar,sair]);
